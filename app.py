...
def stats():
    ...
    for m in por_mes: m['costo'] = round(m['total'] * tarifa)
...
def add_sim():
    ...
    # Check if running on Vercel (read‑only filesystem). Vercel sets the VER​CEL env var.
    if os.getenv('VERCEL'):
        # In serverless mode we cannot write to the SQLite file.
        # Return the computed values without persisting them.
        return jsonify({
            'ok': True,
            'kwh_proyectado': proy,
            'ahorro_cop': ahorro,
            'co2_evitado': co2ev,
            'msg': 'Running on Vercel – simulation not saved to database (read‑only filesystem).'
        })
    # Normal local/development mode – persist the simulation.
    get_db().execute(
        "INSERT INTO simulaciones(usuario_id,descripcion,kwh_actual,porcentaje_ahorro,kwh_proyectado,ahorro_cop,co2_evitado) VALUES(?,?,?,?,?,?,?)",
